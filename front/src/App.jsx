...
function App() {
  ...
  const toggleTask = async (id) => {
    ...
    try {
      ...
      setTasks((prevTasks) =>
        prevTasks.map((t) => (t.id === id ? { ...updatedTask, id: t.id } : t))
      );
    } catch (error) {
  ...
  const confirmRemove = async () => {
    ...
    setTasks((prevTasks) =>
      prevTasks.filter((task) => task.id !== taskIdToRemove)
    );
    setShowModal(false);
      ...
      if (e.ctrlKey || e.metaKey) {
        switch (e.key) {
          ...
          case "n":
            ...
            break;
          default:
            break; // <- necessário para passar o ESLint
        }
      }
    };
  ...
  const addTask = async () => {
    ...
    try {
      ...
      setTasks((prevTasks) =>
        prevTasks.map((t) =>
          t.id === tempTask.id ? { ...task, id: task.id || tempTask.id } : t
        )
      );
      // Limpar campos
      ...
      setTasks((prevTasks) =>
        prevTasks.map((t) =>
          t.id === tempTask.id ? { ...tempTask, id: Date.now().toString() } : t
        )
      );
  ...
  return (
    <div className={theme === "dark" ? "dark" : ""}>
      ...
      <div
        className={`min-h-screen flex flex-col items-center justify-start p-3 sm:p-6 transition-colors duration-500 ${
          theme === "dark" ? "bg-dark-chocolate" : "bg-brown-pastel"
        }`}
      >
        <div
          className={`w-full max-w-3xl rounded-2xl shadow-2xl p-4 sm:p-8 backdrop-blur-sm transition-all duration-500 transform hover:scale-[1.01] ${
            theme === "dark"
              ? "bg-cocoa text-cream-text border border-marshmallow/20"
              : "bg-rose-50 text-gray-900 border border-rose-200/30"
          }`}
        >
          {/* Campo de busca */}
          <div className="mb-6">
              ...
              {searchTerm && (
                <button
                  ...
                  className={`absolute right-3 top-1/2 transform -translate-y-1/2 transition-all duration-200 hover:scale-110 ${
                    theme === "dark"
                      ? "text-muted-pink hover:text-strawberry"
          ...
          <div className="mb-6">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`px-3 py-3 rounded-xl transition-all duration-300 text-sm ${
                    showAdvanced
                      ? theme === "dark"
                        ? "bg-bubblegum text-cream-text"
                        : "bg-rose-300 text-white"
                      : theme === "dark"
                      ? "bg-soft-pink text-cocoa hover:bg-marshmallow"
            ...
            {inputError && (
              <div
                className={`text-xs text-center p-2 rounded-lg mb-3 transition-all duration-300 ${
                  theme === "dark"
            ...
            {showAdvanced && (
              <div
                className={`p-4 rounded-xl border transition-all duration-300 ${
                  theme === "dark"
                    ? "bg-cocoa/30 border-marshmallow/20"
            ...
            <div className="flex flex-wrap justify-center gap-2 mb-4">
              <button
                ...
                className={`px-3 py-2 rounded-full text-xs font-medium transition-all duration-300 transform hover:scale-105 ${
                  theme === "dark"
                    ? "bg-soft-pink hover:bg-marshmallow text-cocoa"
                    : "bg-rose-100 hover:bg-rose-200"
                }`}
              >
              ...
              {tasks.some((t) => t.completed) && (
                <button
                  ...
                  className={`px-3 py-2 rounded-full text-xs font-medium transition-all duration-300 transform hover:scale-105 ${
                    theme === "dark"
                      ? "bg-muted-pink/20 hover:bg-muted-pink/30 text-muted-pink"
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mb-4">
              <button
                className={`px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium transition-all duration-300 transform hover:scale-105 shadow-md text-sm sm:text-base ${
                  filter === "all"
                    ? theme === "dark"
                      ? "bg-bubblegum text-cream-text shadow-bubblegum/30"
                      : "bg-rose-200 shadow-rose-200/30"
                    : theme === "dark"
                    ? "bg-soft-pink hover:bg-marshmallow text-cocoa"
                    : "bg-rose-100 hover:bg-rose-200"
                }`}
                      ...
                      className={`px-2 py-1 rounded-full transition-all duration-300 ${
                        searchTerm === cat
                          ? theme === "dark"
                            ...
                            : "bg-rose-300 text-white"
                          : theme === "dark"
                          ? "bg-cocoa/30 text-muted-pink hover:bg-cocoa/50"
              ...
              >
                {searchTerm ? (
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      Que tal adicionar sua primeira tarefa docinha?
                    </p>
                  </div>
                ) : (
                  <div>
                    ...
                    <p>
                      Nenhuma tarefa{" "}
                      {filter === "completed" ? "concluída" : "ativa"}
                    </p>
                  </div>
                )}
              </div>
              ...
              filteredTasks.map((task, index) => (
                <div
                  ...
                  className={`flex items-center justify-between p-3 sm:p-5 rounded-xl cursor-pointer transition-all duration-300 transform hover:scale-[1.02] hover:-translate-y-1 shadow-lg group ${
                    ...
                    task.completed
                      ? theme === "dark"
                        ? "line-through text-muted-pink bg-rose-900/30 opacity-75"
                        : "line-through text-gray-600 opacity-85"
                      : theme === "dark"
                      ? "text-rose-800 hover:shadow-pink-200/30"
                      : "bg-rose-200 hover:bg-rose-300 hover:shadow-rose-300/30"
                  }`}
                  onMouseEnter={(e) => {
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center min-h-[2rem]">
                      {editingTaskId === task.id ? (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* Metadados */}
                          <div
                            className={`flex flex-wrap items-center gap-3 text-xs mt-2 ${
                              theme === "dark"
                                ? "text-stone-700 opacity-90"
                      ...
                      <>
                        <button
                          className={`p-1 sm:p-2 rounded-full transition-all duration-300 transform hover:scale-110 flex-shrink-0 ${
                            theme === "dark"
                              ? "text-amber-800 hover:bg-amber-800/20"
          ...
          {/* Contagem e dicas */}
          <div
            className={`mt-6 sm:mt-8 p-3 sm:p-4 rounded-xl text-center font-medium transition-all duration-300 ${
              theme === "dark"
                ? "text-muted-pink bg-dark-chocolate/30"
                : "text-gray-600 bg-rose-100/50"
            }`}
          >
            ...
            >
              💡 Dicas: Duplo clique para editar • Arraste para reordenar •
              Ctrl+K para buscar • Ctrl+N para nova tarefa
            </div>
          </div>
        </div>
      </div>
      ...
      <ConfirmModal
        ...
        message={`Tem certeza que quer limpar todas as ${
          tasks.filter((t) => t.completed).length
        } tarefas concluídas? 🧽 Esta ação não pode ser desfeita!`}
      />
    </div>
  );
}
